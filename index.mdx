...
<Steps>
  <Step title="Create the customer">
    Create a customer with [`POST /customer/create`](api/customer-create). The response includes a Coinut customer ID and, when applicable, a Sumsub KYC link.
  </Step>
  <Step title="Complete KYC">
    Wait for approval through the Sumsub flow or your shared due diligence process. Use [`GET /customer/detail`](api/customer-detail) or the `CUSTOMER_APPROVED` webhook to confirm status.
  </Step>

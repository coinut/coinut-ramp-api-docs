...
<Steps>
  <Step title="Prerequisite: get a payId">
    Create a virtual account via [Virtual Accounts](/virtual-accounts/virtual-accounts-guide). The create response only returns the new VA's `id` — immediately call [`GET /virtual-account/detail`](/api-reference/virtual-account/get-fiat-va-detail) <Icon icon="link" size={14} /> to read `payId`.
  </Step>
...
<Note>
  A virtual account's deposits only auto-convert to crypto if it's linked to an onramp flow. If you only need to receive and reconcile fiat, no autoramp flow is required — see [Autoramp Flows](/transactions/autoramp-flows) and [Onramp Integration](/guides/onramp-integration).
</Note>
...
The `address` you QR-encode must come from a Coinut deposit address record — **not** an arbitrary address you generate yourself. See [Crypto Addresses](/addresses/crypto-addresses#deposit-addresses-offramp-entry) for how it's created and approved; it is **not** available synchronously on creation.
...
- [Virtual Accounts](/virtual-accounts/virtual-accounts-guide) — creating a VA and fetching its `payId`
- [Crypto Addresses](/addresses/crypto-addresses) — deposit address creation and approval
- [Autoramp Flows](/transactions/autoramp-flows) — linking a VA or deposit address to automatic conversion

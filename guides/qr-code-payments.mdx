...
<Steps>
  <Step title="Prerequisite: get a payId">
    Create a virtual account via [Virtual Accounts](/virtual-accounts/virtual-accounts-guide). The create response only returns the new VA's `id` — immediately call [`GET /virtual-account/detail`](/api-reference/virtual-account-detail) <Icon icon="link" size={14} /> to read `payId`.
  </Step>

...
<Steps>
  ...
  <Step title="Add Crypto Address">
    Call [`POST /crypto-address/add`](api/crypto-address-add) with:
  ...
  <Step title="Display VA to Customer">
    Show the virtual account details from [`GET /virtual-account/detail`](api/virtual-account-detail) in your UI. There's a single `bicSwift` field for every currency — it carries the BSB number for AUD and the SWIFT/BIC code for everything else. There's no separate "sort code" field.
  ...
  <Step title="Monitor Deposits">
    ...
    Or poll [`GET /deposit/list`](api/deposit-list) with the customer ID.
  </Step>
  <Step title="Track Trade">
    ...
    Or poll [`GET /trade/list`](api/trade-list).
  </Step>

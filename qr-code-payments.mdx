...
The `address` you QR-encode must come from a Coinut deposit address record ([`POST /deposit-address/create`](api/deposit-address-create), then [`GET /deposit-address/detail`](api/deposit-address-detail)) — **not** an arbitrary address you generate yourself. A newly created deposit address starts in `PENDING` status with `address: null`; wait for the `DEPOSIT_ADDRESS_APPROVED` webhook (or poll [`GET /deposit-address/detail`](api/deposit-address-detail) until `address` is populated) before generating a QR code — there is nothing to encode immediately after creation.
...
<Steps>
  <Step title="Create VA">
    Call [`POST /virtual-account/create`](api/virtual-account-create) with `customerId` and `currency`.
  </Step>
  <Step title="Fetch the PayID">
    The create response only returns the new VA's `id` — it does not include `payId`. Immediately call [`GET /virtual-account/detail`](api/virtual-account-detail)`?id=...` and read `payId` from that response instead.
  </Step>
